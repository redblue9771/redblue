...
时域的基本单元就是“1 秒”，如果我们将一个角频率为![\omega_0](//s0.wp.com/latex.php?latex=%5Comega_0&bg=ffffff&fg=000&s=0 "\omega_0")的正弦波 cos（![\omega_0t](//s0.wp.com/latex.php?latex=%5Comega_0t&bg=ffffff&fg=000&s=0 "\omega_0t")）看作基础，那么频域的基本单元就是![\omega_0](//s0.wp.com/latex.php?latex=%5Comega_0&bg=ffffff&fg=000&s=0 "\omega_0")。
...
eix=cosx+isinxeix=cosx+isinxe^{ix}=cosx+isinx

这个公式在数学领域的意义要远大于傅里叶分析，但是乘它为宇宙第一耍帅公式是因为它的特殊形式——当 x 等于![\pi](//s0.wp.com/latex.php?latex=%5Cpi&bg=ffffff&fg=000&s=0 "\pi")的时候。

eiπ+1=0eiπ+1=0e^{i\\pi}+1=0
...
eiπ=cos(t)+i.sin(t)eiπ=cos(t)+i.sin(t)e^{i\\pi}=cos(t)+i.sin(t)  
e−iπ=cos(t)−i.sin(t)e−iπ=cos(t)−i.sin(t)e^{-i\\pi}=cos(t)-i.sin(t)  
将以上两式相加再除 2，得到：  
cos(t)=eiπ+e−iπ2cos(t)=eiπ+e−iπ2cos(t)=\\frac{e^{i\\pi+e^{-i\\pi}}}2  
这个式子可以怎么理解呢？
